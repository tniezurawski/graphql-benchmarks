...
const doBench = async (opts, handler) => {
  ...
  let forked
  try {
    await access(join(__dirname, '..', 'nodejs-benchmarks', handler + '.cjs'))
    forked = fork(join(__dirname, '..', 'nodejs-benchmarks', handler + '.cjs'))
  } catch {
    forked = fork(join(__dirname, '..', 'nodejs-benchmarks', handler + '.mjs'))
  }
